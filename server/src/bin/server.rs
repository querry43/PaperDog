...
impl Handler for Server {
    ...
    fn on_message(&mut self, msg: ws::Message) -> ws::Result<()> {
        let m = paper_dog_server::Message::from(msg.as_text()?);
		println!("on_message: {:?}", m);
        let mut r = self.robot.lock().unwrap();

        let res = match m {
            paper_dog_server::Message::PWMChannelState(msg) => r.update_pwm_channel(msg),
            paper_dog_server::Message::ShiftRegisterState(msg) => r.update_shift_register(msg),
            paper_dog_server::Message::RobotSpeak(msg) => r.robot_speak(msg),
        };
